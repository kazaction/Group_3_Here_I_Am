...
const CvGeneration = () => {
  // Component State
  const [step, setStep] = useState(0); // Tracks the current question index
  const [input, setInput] = useState(""); // Holds the value for text inputs
  const [file, setFile] = useState(null); // Holds the file for upload
  const [error, setError] = useState(""); // Stores error messages from the backend
  const [answers, setAnswers] = useState({}); // Accumulates user's answers
    ...
    try {
      ...
      if (current.field === "picture_path") {
        ...
        if (!data.ok) {
          ...
          setAnswers((prev) => ({
            ...prev,
            // Save a server-relative path, not the full system path
  ...
  // 🔹 NEW: Download CV as PDF when all questions are done
